...
import { Plus, Search, FileDown, TrendingUp, DollarSign, Users, Clock, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function SOP1Cotizaciones() {
  ...
  // Calculate KPIs
  const totalPendientes = filtered.filter(c => c.status === 'pending' || c.status === 'draft').length;
  const totalEnProgreso = filtered.filter(c => c.status === 'inProgress').length;
  const totalCompletados = filtered.filter(c => c.status === 'approved').length;
  const totalFueraSLA = filtered.filter(c => c.sla === 'rojo').length;
